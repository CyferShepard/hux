...
class HuxDateRangePicker extends StatefulWidget {
  /// Creates a Hux date picker dropdown anchored to the trigger button.
  const HuxDateRangePicker(
      {super.key,
  ...
  /// The initially selected end date. If null, the button shows [placeholder].
  ...
  @override
  State<HuxDateRangePicker> createState() => _HuxDateRangePickerState();
}

class _HuxDateRangePickerState extends State<HuxDateRangePicker> {
  ...
  @override
  void didUpdateWidget(covariant HuxDateRangePicker oldWidget) {
          ...
          children: [
            ...
            CompositedTransformFollower(
              ...
              child: Material(
                ...
                child: _HuxDateRangePickerPanel(
                  ...
                  onSelected: (date) {
                    setState(() {
                      _currentStartDate = date.start;
                      _currentEndDate = date.end;
                    });
  ...
  Widget build(BuildContext context) {
    final String label = '${_formatDate(context, _currentStartDate)} - ${_formatDate(context, _currentEndDate)}';

    // Use ghost variant (no border, no background, no padding) for icon-only mode
    final buttonVariant = widget.showText ? widget.variant : HuxButtonVariant.ghost;
...
class _HuxDateRangePickerPanel extends StatefulWidget {
  const _HuxDateRangePickerPanel({
    required this.initialStartDate,
    required this.initialEndDate,
    required this.firstDate,
    required this.lastDate,
  ...
  @override
  State<_HuxDateRangePickerPanel> createState() => _HuxDateRangePickerPanelState();
}

class _HuxDateRangePickerPanelState extends State<_HuxDateRangePickerPanel> {
  ...
  void initState() {
    ...
    final int defaultYear =
        widget.initialStartDate.year >= 1900 && widget.initialStartDate.year <= 2050 ? widget.initialStartDate.year : 2025;
    _currentMonth = DateTime(defaultYear, widget.initialStartDate.month);
  ...
  bool setStartDate = true;
  void _handleSelect(DateTime date) {
    if (setStartDate && date.isAfter(_selectedEndDate) || !setStartDate && date.isBefore(_selectedStartDate)) {
      setStartDate = false;
    }
    if (setStartDate) {
      setState(() {
    ...
    widget.onSelected(DateTimeRange(start: _selectedStartDate, end: _selectedEndDate));
  }
            ...
            child: Row(
              children: List.generate(7, (dayIndex) {
                ...
                if (isPrevMonth) {
                  ...
                  final DateTime date = DateTime(_currentMonth.year, _currentMonth.month, dayNumber);
                  final bool isSelected = (date.year == _selectedStartDate.year &&
          ...
          child: Center(
            child: Text(
              ...
              style: TextStyle(
                color: _getTextColor(),
                fontSize: 14,
                fontWeight: widget.isSelected || widget.isToday ? FontWeight.w600 : FontWeight.w500,
              ),
            ),
          ),
        ),
